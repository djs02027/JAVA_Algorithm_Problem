...
public class D2_1946X {
	public static void main(String args[]) throws Exception {
		...
		for (int test_case = 1; test_case <= T; test_case++) {
			int alpa_num = sc.nextInt();
			char[] ch = new char[alpa_num];
			int[] num = new int[alpa_num];
			
			int ch_count = 0;
			int n=1;
			// 입력
			for (int i = 0; i < alpa_num; i++) {
				ch[i] = sc.next().charAt(i);
				num[i] = sc.nextInt();
			}

			
			char[][] ch_field = new char[(ch_count / 2) + 1][10];
